...
NAMESPACE_APACHE="default"
NAMESPACE_TEST="monitoring"

#echo "1. Creating namespace..."
#kubectl create namespace $NAMESPACE_APACHE --dry-run=client -o yaml | kubectl apply -f -
...
#while true; do
#    EXTERNAL_IP=$(kubectl get svc apache-svc -n $NAMESPACE_APACHE --output=jsonpath='{.status.loadBalancer.ingress[0].ip}')
...
echo "5.1 Delete k6 script configmap..."
...
echo "
=== 👀 [ DEBUG ] ==="
POD=$(kubectl get pods -n $NAMESPACE_TEST -l job-name=k6-job-load-test -o jsonpath='{.items[0].metadata.name}')

echo "9. Streaming k6 logs...
[!] kubectl logs -f $POD -n $NAMESPACE_TEST
